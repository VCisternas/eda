...
```
Interpretation: Wie auch im Scatterplot ist zu erkennen, dass die beiden Merkmale Groesse und Schuhgroesse eine starke (~ 0,7) positive Korrelation haben und das Merkmal Anz.Paar.Schuhe jeweils zu den beiden anderen Merkmalen eine leicht negative (~ -0,07 und ~ -0.25) Korrelation aufweist. Ob dadurch davon ausgegangen werden kann, dass kleinere Schuhe günstiger sind und deshalb öfter gekauft werden ist fragwürig.
...
parcoord(threeD.df, var.label = TRUE) #Anz.Paar.Schueh in Mitte des Plots
```
Interpretation:
Ist das Merkmal in der Mitte des Parallel Coordinate Plots ist gut zu erkennen, dass zu den Merkmalen Groesse und Schuhgroesse nur eine geringe Korrelation besteht, da die Verbindungen keinem Muster folgen. Auch hier ist zu erkennen, dass 4 der 5 größten Werte des Merkmals Anz.Paar.Schuhe zu relativ kleinen Werten der beiden anderen Merkmale verbunden sind, sodass eine leicht negative Korrelation angenommen werden kann.
Ist das Merkmal Groesse in der Mitte des Plots. ist die positive Korrelation zwischen der Groesse und der Schuhgroesse erkennbar, da es nur wenige Linien gibt, die nicht dem Muster entsprechen, welches eine positive Korrelation beschreibt.
