...
```
b)
Die Beziehung zwischen der Beschleunigungskraft und der vergangenen Zeit lässt sich nicht gut linear ausdrücken. Dies ist auf dem Scatterplot, sowie vorallem in der residuen Anbalyse zu erkennen. Bei dem Histogram "Verteilung der Messzeitpunkte" fällt außerdem auf, dass bei ca. 15ms nach dem Aufprall besonders viele Messungen stattgefunden haben.
```{r}
par(mfrow=c(2,2))
...
with(df, {
  plot(Year, deviation)
  lines(ksmooth(Year, deviation, kernel = 'box', bandwidth= 10))
})

help(ksmooth)

ksmooth(df$Year, df$deviation, kernel = 'box', bandwidth= 100)
