...
hist(a)
hist(b)
#~~Beobachtung: diff.med(a) variiert viel mehr als diff.med(b)~~
...
sim.v <- function(a, n){
  ...
  x <- sort(x, decreasing = FALSE) # necessar to draw plot
  df <- data.frame()
  for(z in x){
    v <- exp(1)^z+2*z^2
    df <-rbind(df, list(z, v)) 
  }
  plot(df, type = "b", xlab = "X", ylab = "V") #not required
  return(quantile(df[[2]], a))
}
