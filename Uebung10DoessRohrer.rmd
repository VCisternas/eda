...
Das Merkmal Musikalitaet ist im Gegensatz zum Merkmal Fussballfan nicht binär und kann somit mehr als 2 verschiedene Werte annehmen. In der Analyse ist dabei zu berücksichtigen, dass es keine Vier-Felder-Tafel gibt. Daher muss bei der Interpretation des Mosaic Plots darauf geachtet werden, dass das Merkmal Musikalitaet evtl nicht in richtiger Reihenfolge sortiert ist.
```{r}
abs_table2 <- structable(data2019$Geschlecht ~ data2019$Musikalitaet)
abs_table2
rel_table2 <- prop.table(abs_table)
rel_table2
mosaicplot(abs_table2, xlab = "Musikalitaet", ylab = "Geschlecht", off = 5, main = "Mosaic: Geschlecht~ Musiaklitaet")
```
Es ist kein deutlicher Zusammenhang zwischen Geschlecht und Musikalitaet erkennbar.
...
abs_table3
rel_table3 <- prop.table(abs_table)
rel_table3
